...
class Kategori {
    ...
    public function getDataById($kategori_id){
    ...
    public function updateData($kategori_id, $data){
        // query untuk update data
        $query = $this->db->prepare("update {$this->table} set kategori_nama = ? where kategori_id = ?");

        // binding parameter ke query
        $query->bind_param('si', $data['kategori_nama'],  $kategori_id);

        // eksekusi query
        $query->execute();
    }

    public function deleteData($kategori_id){
